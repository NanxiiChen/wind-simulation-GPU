...
from stochastic_wind_simulate.numpy_backend.simulator_nonstationary import NumpyNonstationaryWindSimulator
...
def _default_output_path(mode):
    ...
    return Path("benchmark_results") / f"nonstationary_benchmark_numpy_{mode}_{timestamp}.csv"
...
def main():
    parser = argparse.ArgumentParser(description="Run NumPy nonstationary wind simulation")
    ...
    simulator = NumpyNonstationaryWindSimulator(key=42, spectrum_type="kaimal-nd")
    ...
    logging.info("Running %d NumPy nonstationary benchmark case(s)", len(combinations))
    ...
    with output_path.open("w", newline="", encoding="utf-8") as csv_file:
        ...
        for n_points, n_freqs in combinations:
            logging.info(
                "Running NumPy nonstationary simulation for n_points=%d, n_freqs=%d",
                n_points,
                n_freqs,
            )
            ...
            wind_speeds = np.full((n_points,), 30.0, dtype=np.float64)
